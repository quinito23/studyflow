...
async function validarDuplicados(correo, contrasenia, DNI) {
    try {
        const response = await new Promise((resolve, reject) => {
            ...
            xhr.open('GET', `/studyflow/APIs/registro_api.php?${params.toString()}`, true);
...
async function validarDuplicadosProfesor(correo, contrasenia, DNI, id_usuario = null) {
    try {
        const response = await new Promise((resolve, reject) => {
            ...
            xhr.open('GET', `/studyflow/APIs/profesor_api.php?${params.toString()}`, true);
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4) {
                    if (xhr.status === 200) {
                        resolve(JSON.parse(xhr.responseText));
...
async function validarDuplicadosAlumno(correo, contrasenia, DNI, id_usuario = null) {
    try {
        const response = await new Promise((resolve, reject) => {
            ...
            if (DNI) params.append('DNI', DNI);
            if (id_usuario) params.append('id_usuario', id_usuario);
            xhr.open('GET', `/studyflow/APIs/alumno_api.php?${params.toString()}`, true);
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4) {
                    if (xhr.status === 200) {
                        resolve(JSON.parse(xhr.responseText));
